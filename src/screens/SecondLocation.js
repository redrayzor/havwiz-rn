...
export default function SecondLocation({navigation}) {
    ...
    return (
      <View>
        ...
        <Pressable
          ...
          onPress={() => navigation.navigate('DistanceCalculation')}>
  ...
  return (
    <ScrollView style={styles.outerBox}>
      <Text style={styles.h1}>Input Second Location{'\n'}</Text>
      <Text style={styles.h2}>Select method</Text>
      <GpsApiSwitch />
      {switchLeft ? (
        <GpsInterface screenIndex={1} />
      ) : (
        <ApiUi screenIndex={1} />
      )}
      <NavFooter />
